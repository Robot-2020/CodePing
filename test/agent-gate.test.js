...
describe("isAgentEnabled", () => {
  it("returns true when snapshot is missing", () => {
    assert.strictEqual(isAgentEnabled(null, "comate"), true);
    assert.strictEqual(isAgentEnabled(undefined, "comate"), true);
    assert.strictEqual(isAgentEnabled({}, "comate"), true);
  });
  ...
  it("returns true when agents field is absent (legacy upgrade)", () => {
    assert.strictEqual(isAgentEnabled({ lang: "en" }, "comate"), true);
  });
  ...
  it("returns true when enabled is not false", () => {
    const snap = { agents: { comate: { enabled: true } } };
    assert.strictEqual(isAgentEnabled(snap, "comate"), true);
  });

  it("returns false only when enabled === false", () => {
    assert.strictEqual(
      isAgentEnabled({ agents: { comate: { enabled: false } } }, "comate"),
      false
    );
  });

  it("treats malformed agent entries as enabled", () => {
    assert.strictEqual(isAgentEnabled({ agents: { comate: "nope" } }, "comate"), true);
    assert.strictEqual(isAgentEnabled({ agents: { comate: null } }, "comate"), true);
  });
});
...
describe("setAgentFlag command", () => {
  ...
  it("rejects unknown flag name", () => {
    ...
    const r = commandRegistry.setAgentFlag(
      { agentId: "comate", flag: "wombat", value: false },
      deps
    );
    assert.strictEqual(r.status, "error");
  ...
  it("rejects non-boolean value", () => {
    const { deps } = makeDeps();
    const r = commandRegistry.setAgentFlag(
      { agentId: "comate", flag: "enabled", value: "off" },
      deps
    );
    assert.strictEqual(r.status, "error");
  });
  ...
  it("is noop when value already matches current state", () => {
    ...
    const r = commandRegistry.setAgentFlag(
      { agentId: "comate", flag: "enabled", value: true },
      deps
    );
  ...
  it("disabling master flag runs stop + clear sessions + dismiss permissions", () => {
    const { deps, calls } = makeDeps();
    const r = commandRegistry.setAgentFlag(
      { agentId: "comate", flag: "enabled", value: false },
      deps
    );
    assert.strictEqual(r.status, "ok");
    assert.deepStrictEqual(calls.stopMonitorForAgent, ["comate"]);
    assert.deepStrictEqual(calls.clearSessionsByAgent, ["comate"]);
    assert.deepStrictEqual(calls.dismissPermissionsByAgent, ["comate"]);
    assert.strictEqual(calls.startMonitorForAgent.length, 0);
    assert.strictEqual(r.commit.agents.comate.enabled, false);
  ...
  it("enabling a previously-disabled agent starts the monitor", () => {
    ...
    seeded.agents.comate = { enabled: false, permissionsEnabled: true };
    ...
    assert.strictEqual(r.status, "ok");
    assert.deepStrictEqual(calls.startMonitorForAgent, ["comate"]);
    assert.strictEqual(calls.stopMonitorForAgent.length, 0);
    assert.strictEqual(r.commit.agents.comate.enabled, true);
  });
  ...
  it("missing side-effect deps are tolerated (simulates hook-only agent)", () => {
    // The simplified runtime has no background monitors — the command should
    // still succeed and only update persisted agent flags.
    const { deps } = makeDeps();
