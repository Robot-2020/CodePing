...
describe("prefs.getDefaults", () => {
  ...
  it("seeds supported agents as enabled", () => {
    const d = prefs.getDefaults();
    for (const id of ["claude-code", "comate"]) {
      assert.strictEqual(d.agents[id].enabled, true, `${id} should default enabled`);
    }
  });

  it("seeds supported agents with permissionsEnabled=true", () => {
    const d = prefs.getDefaults();
    for (const id of ["claude-code", "comate"]) {
      assert.strictEqual(
  ...
  it("normalizes agents (drops malformed entries)", () => {
    const v = prefs.validate({
      agents: {
        ...
        "comate": { enabled: "true" }, // wrong type — should be dropped
      },
    });
    assert.strictEqual(v.agents["claude-code"].enabled, false);
    // bogus + bad Comate entry use defaults
    assert.strictEqual(v.agents.comate.enabled, true);
