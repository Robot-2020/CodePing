...
function makeServer(overrides = {}) {
  ...
  const ctx = {
    ...
    syncComateHooksImpl: () => syncCalls.push("comate"),
    ...overrides,
  };
...
describe("server Claude hook management", () => {
  it("startup syncs Claude hooks and starts watcher when automatic management is enabled", () => {
    ...
    assert.deepStrictEqual(syncCalls, ["claude", "comate"]);
    assert.ok(getWatcher(), "watcher should start when management is enabled");
  });

  it("startup skips Claude sync/watcher but still syncs Comate when automatic management is disabled", () => {
    const { api, syncCalls, getWatcher } = makeServer({
    ...
    assert.deepStrictEqual(syncCalls, ["comate"]);
  ...
  it("disconnect-style restart does not reinstall Claude hooks when management stays disabled", () => {
    ...
    assert.deepStrictEqual(first.syncCalls, ["comate"]);
    assert.deepStrictEqual(second.syncCalls, ["comate"]);
  });
});
