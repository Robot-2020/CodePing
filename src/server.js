...
function syncComateHooks() {
  try {
    if (typeof ctx.syncComateHooksImpl === "function") return ctx.syncComateHooksImpl();
      ...
      req.on("end", () => {
        ...
        try {
          ...
          if (data.agent_id === "opencode") {
            ...
            const isComate = !!data.is_comate;
            const agentName = isComate ? "comate" : "opencode";
            ...
            ctx.updateSession(sessionId, "notification", "PermissionRequest", { agentId: agentName });
          ...
          // Claude Code, since it shares this path) falls back to its built-in
