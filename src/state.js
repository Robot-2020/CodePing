...
function detectRunningAgentProcesses(callback) {
  ...
  if (process.platform === "win32") {
    exec(
      'wmic process where "(Name=\'node.exe\' and (CommandLine like \'%claude-code%\' or CommandLine like \'%comate%\')) or Name=\'claude.exe\' or Name=\'comate.exe\'" get ProcessId /format:csv',
      { encoding: "utf8", timeout: 5000, windowsHide: true },
      (err, stdout) => done(!err && /\d+/.test(stdout))
    );
    return;
  }
  exec("pgrep -f 'claude|comate'", { timeout: 3000 }, (err) => done(!err));
}
