...
function delegateWindowToStationarySpace(nsWindow) {
  try {
    ...
    if (!warnedSkyLightFailure) {
      console.warn("CodePing: failed to move macOS window into stationary SkyLight space:", err.message);
...
function applyStationaryCollectionBehavior(browserWindow) {
  ...
  try {
    ...
    if (!warnedApplyFailure) {
      console.warn("CodePing: failed to apply macOS stationary window behavior:", err.message);
