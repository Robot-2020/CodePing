...
function getActionablePermissions() {
  return pendingPermissions.filter(
    p => !p.isElicitation && p.toolName !== "ExitPlanMode"
  );
}
...
function showPermissionBubble(permEntry) {
  ...
  bub.webContents.once("did-finish-load", () => {
    ...
    bub.webContents.send("permission-show", {
      ...
      isComate: permEntry.isComate || false,
...
// while opencode skips the bridge reply so its terminal prompt can take over.
function dismissPermissionsByAgent(agentId) {
  ...
  for (const perm of toDismiss) {
    ...
    // CC / elicitation: destroy the connection so Claude Code falls back
...
function cleanup() {
  ...
  for (const perm of [...pendingPermissions]) {
    ...
    resolvePermissionEntry(perm, "deny", "CodePing is quitting");
  }
}
