...
Clawd 桌宠 — 一个 Electron 桌面宠物，通过 hook 系统和本地状态同步实时感知 AI coding agent 的工作状态并播放对应的像素风动画（SVG / APNG / GIF / PNG 等）。当前仓库聚焦 **Claude Code**（command + HTTP hook）与 **Comate**（command hook + 本地 monitor）两种使用场景。内置两套主题 **Clawd**（像素螃蟹）与 **Calico**（三花猫），并支持用户自定义主题。支持 Windows、macOS 和 Linux，UI 三语（en / zh / ko）。
...
npm run install:comate-hooks   # 注册 Comate hooks
...
单元测试当前主要覆盖 Claude / Comate 相关 registry、prefs、hook 注册和端口发现逻辑（如 `test/registry.test.js`、`test/server-hook-management.test.js`、`test/install.test.js`、`test/server-config.test.js`），使用 Node.js 内置 test runner。Electron 主进程（状态机、窗口、托盘）仍以自动化测试 + 手动验证结合为主。
