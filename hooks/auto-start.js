  ...
  try {
    if (isPackaged) {
      if (isWin) {
        ...
        // exe:       <install>/CodePing.exe
        const installDir = path.resolve(__dirname, "..", "..", "..");
        const exe = path.join(installDir, "CodePing.exe");
        ...
        // deb/dir:  executable is <install>/codeping, same depth as Windows.
        //   __dirname: <install>/resources/app.asar.unpacked/hooks
        //   install:   3 levels up
        ...
        if (appImage) {
          ...
          const installDir = path.resolve(__dirname, "..", "..", "..");
          const exe = path.join(installDir, "codeping");
          spawn(exe, [], { detached: true, stdio: "ignore" }).unref();
        }
      }
    } else {
